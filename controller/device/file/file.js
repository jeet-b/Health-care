...
function makeFileController({ fileService, makeFile }) {
    ...
    try {
      if (!req.files) {
        ...
        if (Array.isArray(files) === false) {
          const data = await fileData(files, folder, viewType);
          result.push(await fileService.createDocument(data));
        } else {
          //  result = [];
          await Promise.all(
            _.map(files, async (file) => {
              const data = await fileData(file, folder, viewType);
              result.push(await fileService.createDocument(data));
            })
          );
        }
  ...
  const fileData = async (file, folder, viewType) => {
    if (!file.name.match(/\.(mp4|MP4|PDF|pdf|jpg|JPG|jpeg|JPEG|png|PNG)$/)) {
      throw new Error("Only selcted fomat files are allowed..");
    }
    if (file.size / (1024 * 1024).toFixed(2) > 5) {
      utils.failureResponse(
        { message: "Your image size is more than 5 mb" },
        res
      );
    }
